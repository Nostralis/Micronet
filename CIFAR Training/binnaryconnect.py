...
class BC():
    ...
    def binarization(self):
        ...
        for index in range(len(self.target_modules)):
            self.target_modules[index]=torch.where(self.target_modules[index]>0,1,-1)
    ...
    def clip(self):
        ...
        hth = nn.Hardtanh(-1,1)
        for index in range(self.num_of_params):
                self.target_modules[index]=hth(self.target_modules[index])

    def affic(self):
        i=0
        for m in self.model.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                print(self.target_modules[i])
                i+=1
